...
from reminder_queue import enqueue_reminder, ensure_daemon_running
...
class ReminderAlert(QWidget):
    ...
    def _snooze(self, minutes: int) -> None:
        enqueue_reminder(self.title_text, self.body_text, self.severity, delay_seconds=max(1, minutes) * 60)
        ensure_daemon_running(entry_command(HERE / "reminder_daemon.py"))
        self.close()
...
def main(argv: list[str] | None = None) -> int:
    ...
    app.setApplicationName("Hanauta Reminder Alert")
