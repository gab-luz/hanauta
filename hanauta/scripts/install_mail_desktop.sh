...
DESKTOP_TARGET="$HOME/.local/share/applications/hanauta-mail.desktop"
INSTALL_MAILTO=false

for arg in "$@"; do
  case "$arg" in
    --mailto) INSTALL_MAILTO=true ;;
  esac
done
...
if [ "$INSTALL_MAILTO" = true ]; then
  if command -v xdg-mime >/dev/null 2>&1; then
    xdg-mime default hanauta-mail.desktop x-scheme-handler/mailto >/dev/null 2>&1 || true
  fi
  if command -v xdg-settings >/dev/null 2>&1; then
    xdg-settings set default-url-scheme-handler mailto hanauta-mail.desktop >/dev/null 2>&1 || true
  fi
fi
