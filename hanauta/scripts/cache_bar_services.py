...
CACHE_FILE = HOME / ".local" / "state" / "hanauta" / "service" / "plugins" / "bar-services.json"
PLUGIN_ENTRYPOINT = "hanauta_plugin.py"
BAR_ENTRYPOINT = "hanauta_bar_plugin.py"
...
def _discover_plugin_dirs(settings: dict[str, object]) -> list[Path]:
    ...
    for root in _plugin_search_roots(settings):
        if not root.exists() or not root.is_dir():
            continue
        if root.name == "dev":
            children = sorted([p for p in root.iterdir() if p.is_dir() and p.name.startswith("hanauta-plugin-")])
        else:
            children = sorted([p for p in root.iterdir() if p.is_dir()])
        for child in children:
            if not (child / PLUGIN_ENTRYPOINT).exists():
                continue
            if not (child / BAR_ENTRYPOINT).exists():
                continue
...
def _collect_rows_from_plugin(plugin_dir: Path) -> list[dict[str, object]]:
    ...
    module_name = f"hanauta_plugin_cache_{hash(str(entrypoint)) & 0xFFFFFFFF:x}"
    ...
    for section in sections:
        ...
        if not key:
            continue
        supports_show = bool(section.get("supports_show_on_bar", False))
        if not supports_show:
            continue
        ...
        rows.append(
            {
                "key": key,
                "label": label,
                "icon": _safe_icon_name(section.get("icon_name", section.get("icon", "widgets"))),
                "supports_show_in_bar": True,
                "plugin_id": plugin_id,
